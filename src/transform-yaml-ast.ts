...
/**
 * YAML AST anchor/alias mechanics for the Markdown format driver.
 *
 * This module owns the pure YAML AST utilities the frontmatter driver needs:
 * scalar/key resolution, alias-use analysis, anchor stripping, unresolved-alias
 * rejection, alias cloning, and the cwd/parentSession alias-isolation passes.
 * It carries no path-rewrite semantics and no diagnostic vocabulary: every
 * helper here operates on the YAML document structure alone, so alias behavior
 * can be reasoned about without the visitor, the scanners, or the sync
 * orchestration modules.
 *
 * Dependency direction is one-way: this module imports only `yaml` and
 * standard TypeScript primitives. `transform-yaml.ts` imports these helpers,
 * and nothing here may import the format driver, `transform.ts`, the scanners,
 * or the orchestration modules.
 */
import {
  type Alias,
  type Document,
...
export function yamlStringValue(node: unknown, document: Document): string | undefined {
...
export function resolvedYamlScalar(
  node: unknown,
  document: Document,
...
export function rejectUnresolvedYamlAliases(document: Document): void {
 ...
 */
export function isolateSharedYamlCwdAliases(document: Document): void {
 ...
 */
export function isolateSharedYamlParentSessionAliases(document: Document): void {
  const analysis = analyzeYamlAliases(document);
  for (const [source, entries] of analysis.uses) {
    const anchored = analysis.anchoredNodes.get(source);

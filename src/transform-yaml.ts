 ...
 *
 * This module owns the whole Markdown/YAML half of the transform pipeline:
 * frontmatter detection and parsing, YAML `cwd` conversion, the YAML
 * scalar/mapping/sequence walkers (cwd rewriting, generic path rewriting,
 * path-reference collection, parentSession validation), anchor/alias analysis,
 * isolation and cloning, YAML node diagnostic location, and the final
 * Markdown transform and rendering.
 *
 ...
 * diagnostic mechanics (`transform-diagnostics.ts`), and the portable-name /
 * session-path / sync-path helpers it needs. It MUST NOT import
 * `transform.ts`, the scanners, or the orchestration modules.
 */
import {
  type Alias,
  type Document,

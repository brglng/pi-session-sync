...
import { commitSyncPlan } from "./sync-commit-runner.ts";
import {
  deleteDecision,
...
import { RealtimeSyncReporter, SCAN_EVENT_KEY } from "./sync-events.ts";
import {
...
async function syncSessionsInternal(
  ...
  try {
    ...
    try {
      ...
      const { copied, deleted } = await commitSyncPlan({
        reporter,
        ctx,
        sessionsTreeFrozen,
        commitDecisions,

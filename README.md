...
- v0.4.1 leniency: a path value that cannot be encoded or decoded as a portable path is preserved verbatim with a warning instead of stopping the whole sync, in BOTH directions. This covers `cwd`, `parentSession`, and generic sessions/missions path fields; a value need not be absolute. Relative values and identifiers that are not path candidates stay unchanged. Malformed structured syntax, wrong field types, and unsafely parsed sync URIs remain hard errors.
...
- Local-to-target `parentSession` values keep the parent-file contract: a sessions-directory URI, a referenced target that exists as a non-regular file, and a malformed or loose-spelled sync URI are file errors that stop the sync before staging. A value that cannot be encoded as a portable session path (Windows-shaped/UNC, out-of-root, missions-root, or an in-root path without a mapping) is preserved verbatim with a warning instead of stopping the sync.
...
- v0.4.1: a path-field value that cannot be encoded as a portable path is preserved verbatim with a warning instead of stopping the sync, in both directions. The original value need not be absolute; only malformed structured syntax, wrong field types, and unsafely parsed sync URIs stay hard errors.
...
- Any `pi-session-sync:` prefix must be a valid case-insensitive `pi-session-sync://` URI (strict for local sources). A path value that cannot be encoded/decoded as a portable path — a relative `cwd`, a target spelling that is not portable, or an in-root path without a mapping — is preserved verbatim with a warning in both directions.
- Local absolute `parentSession` paths inside `sessionsRoot` become `pi-session-sync://sessions/<portableName>/<relativePath>`; relative values remain unchanged and reverse URIs restore locally. A parentSession that cannot be encoded (Windows-shaped/UNC, out-of-root, missions-root, or unmapped) is preserved verbatim with a warning. Old rootless file URIs, sessions-directory URIs, and non-regular referenced targets are still errors.
...
- POSIX treats Windows drive and UNC-shaped absolute parents as nonportable: they are preserved verbatim with a warning. Flat absolute parents use their own exact or containing mapping, never the current file’s mapping. A flat absolute parent without an unambiguous mapping is preserved verbatim with a warning.
...
- Frontmatter `parentSession` gets JSONL-equivalent type, URI, and range validation, but its bytes remain unchanged; a nonportable spelling is preserved verbatim with a warning.
...
On target-to-local copies, nonportable values are preserved verbatim with a warning instead of failing the sync: the copied local file keeps the raw target spelling and the sync completes. A later local→target pass is also lenient (v0.4.1): the same nonportable value is preserved verbatim with a warning rather than failing as a file error. Remove or fix the nonportable values on the target side to encode them again.
...
- `sessionsRoot` and the `<agentDir>/missions` root may be symlinks (source roots are followed); `targetDir` must be an existing real, non-symlink directory. The two source roots (sessions ↔ missions), each source root against `targetDir`, and each source root against both target child roots (`targetDir/sessions`, `targetDir/missions`) must not overlap: lexical or real-path overlap is a configuration hard error that stops the sync before any child root, machine identity, or state file is written.
- The one deliberate exception is the validation-created-child race: a source-root symlink whose link target only resolves into a target child root that root validation creates AFTER the real-path overlap check is NOT a hard overlap. The scanner reports it as the nonfatal forbidden-source error in `SyncSummary.errors` and skips it while other safe files keep syncing. Source-vs-source overlap still follows a dangling root symlink and stays a hard configuration error.
...
- Unknown entries, default-root files, and unsupported types are ignored with warnings; unsafe relative segments are errors. Dot-prefixed files and directories never participate in the sync (no read, write, delete, create, warning, or error) in both the local and target trees; the root `pi-session-sync-state.json` is handled by the state rules. A directory that is empty after dot-prefixed entries are excluded is synchronized content (v0.4.2): it is created on the side that lacks it, and a one-sided deletion of a previously synchronized empty directory propagates through its tombstone until both sides agree. Empty directories managed by that sync are never removed by file-deletion cleanup. A flat empty directory whose portable name cannot be derived from a contained file or persisted baseline is reported with a warning instead of being guessed; empty target directories needed by the sync are created normally.
- Local → target `parentSession` references a parent session FILE: a sessions-directory URI (`pi-session-sync://sessions/<portableName>` with no relative path), a referenced target that exists as a non-regular file, or a malformed/loose-spelled URI is a file error before staging. A missing referenced file stays valid when URI, range, and segment rules pass. An absolute parent that cannot be encoded as a portable session path (Windows-shaped/UNC, out-of-root, missions-root, or an in-root path without a mapping) is preserved verbatim with a warning instead of stopping the sync.
